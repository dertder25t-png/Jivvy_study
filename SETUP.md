...
| **B. Real setup** | Sign-in, sync, backup, syllabus reading | Supabase (free) — no AI, no API keys | ~15 min |
...
It asks for the Reference ID, logs you in to Supabase (a browser tab opens — approve it), creates the database and privacy rules, deploys the server functions, and writes the `.env` file. Every step is safe to repeat if something goes wrong.

### 3. Two settings in the Supabase dashboard
...
| Signed in but syllabus upload fails | Re-run `npm run backend:setup` so the server functions are deployed (step 5 of the script). If the file is a scan or a photo, paste the syllabus text instead. |
...
Each account can only ever read its own rows (enforced in the database, not just the app). The app uses no AI service: flashcards are made on your device and syllabi are read by rules on your own Supabase server. **More → Account** lets you download everything or delete the account and all its data permanently.
